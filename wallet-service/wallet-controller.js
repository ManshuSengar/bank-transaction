...
walletRouter.get(
  "/admin/excel-transaction/download",
  authenticateToken,
...
// Get wallet locks
walletRouter.get("/:walletId/locks", authenticateToken, async (req, res) => {
  try {
    const walletId = parseInt(req.params.walletId);
    if (isNaN(walletId)) {
      return res.status(400).send({
    ...
    const userWallets = await walletDao.getUserWallets(req.user.userId);
    const hasAccess =
      userWallets.some((w) => w.wallet.id === walletId) ||
...
walletRouter.get(
  "/transaction-logs/download",
  authenticateToken,
  async (req, res) => {
    try {
      const { type, referenceType, walletType, startDate, endDate, search } =
        req.query;
      ...
      const timestamp = new Date().toISOString().replace(/[:\.]/g, "-");
      const filename = `transaction_logs_${timestamp}.xlsx`;
      const filepath = path.join(
        __dirname,
        "..",
        "uploads",
      ...
      console.error("Error in transaction logs download:", error);
      log.error("Error downloading transaction logs:", error);
      res.status(500).send({
        messageCode: "ERR_DOWNLOAD_TRANSACTION_LOGS",
        message: "Error downloading transaction logs",
  ...
  async (req, res) => {
    try {
      ...
      const result = await walletDao.updateWalletBalance(
        ...
        req.body.reference,
        req.body.userId,
        req.body.referenceType || "ADMIN_DEBIT"
      );

      res.send({
        messageCode: "DEBIT_SUCCESS",
        message: "Amount debited successfully",
        transaction: result.transaction,
      });
    } catch (error) {
      log.error("Error processing debit:", error);
      res.status(error.statusCode || 500).send({
        messageCode: error.messageCode || "ERR_DEBIT",
        message: error.message || "Error processing debit",
      });
    }
  }
);
