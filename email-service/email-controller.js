...
class EmailController {
   ...
   * Uses Ethereal test account in development, production SMTP in production
   */
  async initializeTransporter() {
    try {
      // Create a test account with Ethereal for development environment
      const testAccount = await nodemailer.createTestAccount();

      // Create reusable transporter using appropriate configuration
  ...
  /**
   * Send welcome email to new users with temporary password and setup instructions
   * @param {Object} user - User object containing user details
   * @param {string} temporaryPassword - Temporary password for initial login
   * @returns {Object} Email sending result with message ID and reset token
   */
  async sendWelcomeEmail(user, temporaryPassword) {
    try {
      ...
      // Generate password reset token for first-time setup
      const resetToken = crypto.randomBytes(32).toString("hex");
      const resetExpires = new Date(Date.now() + 24 * 60 * 60 * 1000); // 24 hours expiry
      ...
      // Create password setup link for the user
   ...
   * @returns {Object} Email sending result with success status and message details
   */
  async sendPasswordResetEmail(user, resetToken) {
    try {
      ...
      // Configure email options
      const mailOptions = {
        from: process.env.SMTP_FROM || '"Fonexpay Support" <support@fonexpay.com>',
        to: user.emailId,
      ...
      // Send the email
      const info = await this.transporter.sendMail(mailOptions);
      log.info(`Password reset email sent to ${user.emailId}. MessageId: ${info.messageId}`);
      ...
      const mailOptions = {
        ...
        html: `
          ...
          <ul>
            ...
            <li>Lock Time: ${new Date(user.lockTime).toLocaleString()}</li>
