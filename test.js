...
import {
    ...
    useDeletePfIntCutsByIdMutation // Assuming this mutation exists similarly to tenure
...
const InterestRateWisePortfolioCutsTable = ({ excelData, openSectionsData }: any) => {
    ...
    const [deletePfIntCuts] = useDeletePfIntCutsByIdMutation();
        ...
        if (excelData && excelData?.length > 0) {
            ...
            rateRows.forEach((excelRow: any) => {
                ...
                if (rateSlab !== "Sub Total") {
                    if (rateSlab.includes('-')) {
                        const [minStr, maxStr] = rateSlab.split('-').map((s: string) => s.trim());
                        minRateType = parseFloat(minStr) || 0;
                        maxRateType = parseFloat(maxStr) || 0;
                    } else {
                        minRateType = parseFloat(rateSlab) || 0;
                        maxRateType = minRateType;
                    }
...
const TenureWisePortfolioCuts = ({ excelData, openSectionsData }: any) => {
    ...
    useEffect(() => {
        ...
        if (excelData && excelData?.length > 0) {
            const tenureRows = excelData.filter((row: any) => row[1] && row[1]?.toString().trim() !== 'Sub Total');
            const newData: any = [];
            ...
            tenureRows.forEach((excelRow: any) => {
                ...
                if (tenureSlab !== "Sub Total") {
                    if (tenureSlab.includes('-')) {
                        const [minStr, maxStr] = tenureSlab.split('-').map((s: string) => s.trim());
                        minTenureSlab = parseFloat(minStr) || 0;
                        maxTenureSlab = parseFloat(maxStr) || 0;
                    } else {
                        minTenureSlab = parseFloat(tenureSlab) || 0;
                        maxTenureSlab = minTenureSlab;
                    }
...
import CheckCircleOutlineIcon from '@mui/icons-material/CheckCircleOutline';
import { useAppSelector } from "../../../app/hooks";
import { useGetPFCutsIntWiseFormDataQuery, useSavepFCutsIntWiseFormDetailsMutation } from "../../../features/application-form/Portfoliocuts";
import { AdvanceTextBoxField } from "../../../components/framework/AdvanceTextBoxField";
import { OnlineSnackbar } from "../../../components/shared/OnlineSnackbar";
import ConfirmationAlertDialog from "../../../models/application-form/ConfirmationAlertDialog";
import SaveAsIcon from '@mui/icons-material/SaveAs';
...
const InterestRateWisePortfolioCutsTable = ({ excelData, openSectionsData }: any) => {
    const { applId, transactionData } = useAppSelector((state) => state.userStore);
    const [addSavepFCutsIntWiseFormDetails] = useSavepFCutsIntWiseFormDetailsMutation();
    const { data: getPFCutsIntWiseFormData } = useGetPFCutsIntWiseFormDataQuery(applId, { refetchOnMountOrArgChange: true });
    ...
    useEffect(() => {
        if (getPFCutsIntWiseFormData) {
            setInitialValues({ data: getPFCutsIntWiseFormData });
        }
    }, [getPFCutsIntWiseFormData, applId]);
    ...
    const handleSubmitApis = async (values: any) => {
        ...
        try {
            setIsUploading(true);
            if (await addSavepFCutsIntWiseFormDetails(finalValue).unwrap()) {
                setOpenSnackbar(true);
                setSeverity("success");
                setIsUploading(false);
    ...
    const calculationTotal = (setFieldValue: any, values: any, currentIndex: number, currentVal: number, currentRowId: string) => {
        ...
        const totalPrefix = 'data.13.' + currentRowId;
        setFieldValue(`${totalPrefix}`, total.toFixed(2));
    };
    ...
    const processExcelData = (excelData: any[], formData: any[]) => {
        const mappedData = formData.map((row: any) => {
            const excelRow = findMatchingExcelRow(excelData, row.rateType);
            if (excelRow) {
                const parsedRow = {
                    ...row,
                    tminus2Loans: parseExcelValue(excelRow[1]),
        ...
        return calculateTotalsForExcel(mappedData);
    };
    ...
    const calculateTotalsForExcel = (data: any[]) => {
        const totalRowIndex = data.findIndex(row => row.rateType === "Sub Total");
        if (totalRowIndex !== -1) {
            const fieldsToSum = [
            ...
            fieldsToSum.forEach(field => {
                totalRow[field] = data
                    .filter(row => row.rateType !== "Sub Total")
    ...
    const findMatchingExcelRow = (jsonData: any[], rateType: string): any[] | null => {
        for (let i = 3; i < jsonData.length; i++) {
            const row = jsonData[i];
            if (row && row[0] && row[0].toString().trim() === rateType.trim()) {
                return row;
            }
        }
        if (rateType === "Sub Total") {
            for (let i = 3; i < jsonData.length; i++) {
                const row = jsonData[i];
    ...
    useEffect(() => {
        if (excelData && excelData.length > 0 && getPFCutsIntWiseFormData) {
            const processedData = processExcelData(excelData, getPFCutsIntWiseFormData);
...
as i want InterestRateWisePortfolioCutsTable change like TenureWisePortfolioCuts as in InterestRateWisePortfolioCutsTable now in rate two new things will be add min and max apart from this everything will be same but structure will be like that in api also new fields are added and add and delete functionality like TenureWisePortfolioCuts will be added apart from this api firldf name is same and calucation will be same 

savepFCutsTenurWiseFormDetails payload 
[
  {
    "applId": "string",
    "slNo": 0,
