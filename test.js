...
import {
    useGetPFSanctionWiseFormDataQuery,
    useSavePFSanctionWiseFormDetailsMutation,
    useDeletePFSanctionWiseMutation 
...
const SanctionWisePortfolioCutsTable = ({ excelData, openSectionsData }: any) => {
    const { applId, transactionData } = useAppSelector((state) => state.userStore);
    const [savePFSanctionWiseFormDetails] = useSavePFSanctionWiseFormDetailsMutation();
    const [deletePfSanction] = useDeletePFSanctionWiseMutation();
    const { data: getPFSanctionWiseFormData } = useGetPFSanctionWiseFormDataQuery(applId, { refetchOnMountOrArgChange: true });
    ...
    useEffect(() => {
        if (excelData && excelData?.length > 0) {
            const sanctionRows = excelData.filter((row: any) => {
                const minValue = row[1]; // Min sanction column
            ...
            sanctionRows.forEach((excelRow: any, rowIndex: number) => {
                try {
                    const minSanctionType = parseExcelValue(excelRow[1]); // Min column
                    const maxSanctionType = parseExcelValue(excelRow[2]); // Max column

                    if (minSanctionType !== null && maxSanctionType !== null && !isNaN(minSanctionType) && !isNaN(maxSanctionType)) {
                        ...
                        const rowData = {
                            minSanctionType: Number(minSanctionType.toFixed(2)),
                            maxSanctionType: Number(maxSanctionType.toFixed(2)),
                        ...
                        const existingIndex = newData.findIndex((row: any) => 
                            row.minSanctionType === rowData.minSanctionType && 
                            row.maxSanctionType === rowData.maxSanctionType
                        );
                        if (existingIndex !== -1) {
                        ...
                        console.warn(`Skipping row ${rowIndex}: Invalid sanction values (min: ${minSanctionType}, max: ${maxSanctionType})`);
                    }
                } catch (error) {
    ...
    const sanctionWiseListingSchema = Yup.object().shape({
        data: Yup.array().of(
            Yup.object().shape({
                minSanctionType: Yup.number().required('Required'),
                maxSanctionType: Yup.number().required('Required').moreThan(Yup.ref('minSanctionType'), 'Max must be greater than Min'),
            })
        ),
    });
    ...
    const handleSubmitApis = async (finalValue: any) => {
        try {
            setIsUploading(true);
            const response = await savePFSanctionWiseFormDetails(finalValue).unwrap();
            if (response) {
                setOpenSnackbar(true);
                setSeverity("success");
    ...
    const handleConfirmDelete = async (applId: any, slNo: number) => {
        ...
        try {
            if (await deletePfSanction({ applId, slNo }).unwrap()) {
                setOpenSnackbar(true);
                setSeverity("success");
                setSnackMsg("Record Deleted successfully");
            }
        } catch (error) {
            console.error("Error deleting sanction-wise record:", error);
            setOpenSnackbar(true);
            setSeverity("error");
    ...
    const adjustSubsequentMins = (setFieldValue: any, values: any, startIndex: number, newMax: any) => {
        ...
        for (let i = startIndex + 1; i < values.data.length; i++) {
            setFieldValue(`data.${i}.minSanctionType`, prevMax);
            prevMax = parseFloat(values.data[i].maxSanctionType);
                ...
                <>
                    ...
                    <div className="custome-form">
                        ...
                        <div className="wrap-inner-table" style={{ overflow: 'auto' }}>
                            <Formik
                                initialValues={getPFSanctionWiseFormData || { data: [] }}
                                                            ...
                                                            {values?.data?.[0]?.saveStatus !== "02" && (
                                                                <Button
                                                                    ...
                                                                    onClick={() => {
                                                                        ...
                                                                        let newMin: any = 0;
                                                                        if (lastIndex >= 0) {
                                                                            const lastMax = parseFloat(values.data[lastIndex].maxSanctionType);
                                                                            if (!isNaN(lastMax)) {
                                                                                newMin = lastMax;
                                                                            }
                                                                        }
                                                                        push({
                                                                            minSanctionType: newMin,
                                                                            maxSanctionType: "",
                                                                            tminus2Loans: "",
                                                                            tminus2Pos: "",
                                                                            tminus1Loans: "",
                                                            ...
                                                            <Table sx={{ minWidth: 650 }} aria-label="simple table" className="smTxt">
                                                                <TableHead>
                                                                    <TableRow>
                                                                        ...
                                                                        <TableCell colSpan={2} align='center'><b>Sanction Amount</b></TableCell>
                                                                ...
                                                                <TableBody>
                                                                    {values?.data?.map((item: any, index: any) => (
                                                                        <TableRow key={index}>
                                                                            ...
                                                                            <TableCell>
                                                                                <AdvanceTextBoxField
                                                                                    name={`data.${index}.minSanctionType`}
                                                                                    disabled={true}
                                                                                    type={'number'}
                                                                                    allowNegative={false}
                                                                                    allowDecimal={true}
                                                                            ...
                                                                            <TableCell>
                                                                                <AdvanceTextBoxField
                                                                                    name={`data.${index}.maxSanctionType`}
                                                                                    onCustomChange={(currentVal: any) => {
                                                                                        const val = parseFloat(currentVal) || "";
                                                                                        setFieldValue(`data.${index}.maxSanctionType`, val);
...
})(SanctionWisePortfolioCutsTable);
    ...
    useEffect(() => {
        if (excelData && excelData?.length > 0) {
            const rateRows = excelData.filter((row: any) => {
                const minValue = row[1]; // Min rate column
                return minValue !== undefined &&
                       minValue !== null &&
                       minValue.toString().trim() !== '' &&
                       minValue.toString().trim().toLowerCase() !== 'sub total' &&
            ...
            rateRows.forEach((excelRow: any, rowIndex: number) => {
                try {
                    const minRateType = parseExcelValue(excelRow[1]); // Min column
                    const maxRateType = parseExcelValue(excelRow[2]); // Max column
...
const InterestRateWisePortfolioCutsTable = ({ excelData, openSectionsData }: any) => {
    ...
    useEffect(() => {
        if (excelData && excelData?.length > 0) {
            const rateRows = excelData.filter((row: any) => {
                const slabValue = row[0];
                return slabValue !== undefined &&
                       slabValue !== null &&
                       slabValue.toString().trim() !== '' &&
                       slabValue.toString().trim().toLowerCase() !== 'sub total' &&
                       slabValue.toString().trim().toLowerCase() !== 'subtotal';
            });
            ...
            rateRows.forEach((excelRow: any, rowIndex: number) => {
                try {
                    ...
                    const parts = rateSlab.split('-').map(s => s.trim().replace(/[^0-9.]/g, ''));
                    if (parts.length !== 2) {
                        console.warn(`Skipping row ${rowIndex}: Invalid rate slab format (${rateSlab})`);
                        return;
                    }

                    const minRateType = parseFloat(parts[0]);
                    const maxRateType = parseFloat(parts[1]);

                    if (isNaN(minRateType) || isNaN(maxRateType)) {
                        console.warn(`Skipping row ${rowIndex}: Invalid rate values (min: ${minRateType}, max: ${maxRateType})`);
                        return;
                    }
                    ...
                    const qtrPosFormula = parseExcelValue(excelRow[6]); 
                    const qtrPos = Number((qtrDpd0 + qtrDpd1To30 + qtrDpd31To60 + qtrDpd61To90 + qtrDpdAbove90).toFixed(2));
                    ...
                    if (transactionData?.lstAudQ !== 'Not Applicable') {
                        ...
                        const tposFormula = parseExcelValue(excelRow[13]); 
                        tpos = Number((tdpd0 + tdpd1To30 + tdpd31To60 + tdpd61To90 + tdpdAbove90).toFixed(2));
                    ...
                    const rowData = {
                        minRateType: Number(minRateType.toFixed(2)),
                        maxRateType: Number(maxRateType.toFixed(2)),
                        tminus2Loans: Number(parseExcelValue(excelRow[1]).toFixed(0)),
                        tminus2Pos: Number(parseExcelValue(excelRow[2]).toFixed(2)),
                        tminus1Loans: Number(parseExcelValue(excelRow[3]).toFixed(0)),
                        tminus1Pos: Number(parseExcelValue(excelRow[4]).toFixed(2)),
                        qtrLoans: Number(parseExcelValue(excelRow[5]).toFixed(0)),
                        qtrPos,
                        qtrDpd0: Number(qtrDpd0.toFixed(2)),
                        qtrDpd1To30: Number(qtrDpd1To30.toFixed(2)),
                        qtrDpd31To60: Number(qtrDpd31To60.toFixed(2)),
...
const SanctionedAmountWisePortfolioCutsTable = ({ excelData, openSectionsData }: any) => {
    ...
    useEffect(() => {
        if (excelData && excelData?.length > 0) {
            const amtRows = excelData.filter((row: any) => {
                const slabValue = row[0];
                return slabValue !== undefined &&
                       slabValue !== null &&
                       slabValue.toString().trim() !== '' &&
            ...
            const newData: any = [];
            amtRows.forEach((excelRow: any, rowIndex: number) => {
                try {
                    const amtSlab = excelRow[0]?.toString().trim();
                    const parts = amtSlab.split('-').map(s => s.trim().replace(/[^0-9.]/g, ''));
                    if (parts.length !== 2) {
                        console.warn(`Skipping row ${rowIndex}: Invalid amount slab format (${amtSlab})`);
                        return;
                    }

                    const minAmtSlab = parseFloat(parts[0]);
                    const maxAmtSlab = parseFloat(parts[1]);

                    if (isNaN(minAmtSlab) || isNaN(maxAmtSlab)) {
                        console.warn(`Skipping row ${rowIndex}: Invalid amount values (min: ${minAmtSlab}, max: ${maxAmtSlab})`);
                        return;
                    }
                    ...
                    const rowData = {
                        minAmtSlab: Number(minAmtSlab.toFixed(0)),
                        maxAmtSlab: Number(maxAmtSlab.toFixed(0)),
                        tminus2Loans: Number(parseExcelValue(excelRow[1]).toFixed(0)),
                        tminus2Pos: Number(parseExcelValue(excelRow[2]).toFixed(2)),
                        tminus1Loans: Number(parseExcelValue(excelRow[3]).toFixed(0)),
                        tminus1Pos: Number(parseExcelValue(excelRow[4]).toFixed(2)),
            ...
            if (newData.length > 0) {
                ...
                setOpenSnackbar(true);
                setSeverity("success");
                setSnackMsg(`Sanctioned Amount-wise data imported successfully (${newData.length} rows)`);
            } else {
                setOpenSnackbar(true);
                setSeverity("warning");
                setSnackMsg("No valid sanctioned amount data found in Excel");
            }
        }
    }, [excelData, transactionData]);
