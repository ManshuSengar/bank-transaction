...
class CallbackProcessDao {
  ...
  async processUserCallback(decryptedData, isSettled = false) {
    const context = {
    ...
    try {
      ...
      console.log("error callbac--> ", error);
  ...
  async getFilteredSystemCallbackLogs(filters) {
    try {
      ...
      if (filters.transactionId) {
        conditions.push(
          eq(systemCallbackLogs.transactionId, filters.transactionId)
        );
      }
      ...
      if (filters.search && filters.search.trim()) {
        ...
        switch (filters.searchType) {
          ...
          case "systemTransactionId":
            searchConditions.push(
              sql`CAST(${
                systemCallbackLogs.transactionId
              } AS TEXT) ILIKE ${`%${searchTerm}%`}`
            );
            break;
    ...
    try {
      ...
      return {
        ...
        logs: logs.map((log) => {
          ...
          try {
            modifiedPayload =
              typeof log.modifiedPayload === "string"
