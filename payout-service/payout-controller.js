...
payoutRouter.post("/payout", async (req, res) => {
  try {
    ...
    const { error } = generatePayoutSchema.validate(req.body);
    if (error) {
    ...
    const { username, token, amount, bankAccountNo, ifsc, mobile, beneficiaryName, vpaId } = req.body;
    const clientIp =
      req.ip ||
    ...
    if (!amount || amount < 10 ) {
      return res.status(400).send({
        ...
        message: "Payload must contain valid amount ",
      });
    }

    // Generate Payload for the user
    const result = await payoutDao.generatePayout(
      user.id,
      amount,
      bankAccountNo,
      ifsc,
...
payoutRouter.post("/verify", async (req, res) => {
  try {
    ...
    const result = await payoutDao.verifyTransaction(transactionId);
...
payoutRouter.get("/transactions", async (req, res) => {
  try {
    ...
    const transactions = await payoutDao.getUserpayoutTransactions(user.id, {
    ...
    res.send({
      ...
      message: "User payout transactions retrieved successfully",
      ...transactions,
    });
  } catch (error) {
...
payoutRouter.get("/admin/transactions", async (req, res) => {
  try {
    const {
      startDate,
      endDate,
    ...
    res.send({
      ...
      message: "All user payout transactions retrieved successfully",
      data: transactions.data.map((transaction) => ({
        transactionId: transaction?.transactionId,
        amount: transaction.amount,
        uniqueId: transaction.uniqueId,
...
payoutRouter.post("/check-status", async (req, res) => {
  try {
    ...
    const user = await userDao.getUserByUsername(username);
    const transaction = await payoutDao.getTransactionByUniqueId(uniqueId);
    ...
    if (newStatus !== "PENDING" && newStatus !== transaction.status) {
      await payoutDao.processStatusChange(
        transaction,
        newStatus === "SUCCESS",
        amount,
        statusData.BankRRN
    ...
    const updatedTransaction = await payoutDao.getTransactionByUniqueId(
      uniqueId
    );
    res.send({
...
payoutRouter.post("/admin/check-status", async (req, res) => {
  try {
...
payoutRouter.get("/admin/transactions/download", async (req, res) => {
  try {
    const {
      startDate,
      endDate,
...
payoutRouter.post("/admin/mark-failed", authenticateToken, async (req, res) => {
  try {
    ...
    const updatedTransaction = await payoutDao.processStatusChange(
      transaction,
